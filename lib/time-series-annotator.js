...
$.widget('crowdcurio.TimeSeriesAnnotator', {
    ...
    options: {
        ...
        instructionSlidesURL: undefined,
        features: {
                ...
                <div class="graph_control"> \
                    ...
                    <div class="button_container container-fluid"> \
                        ...
                        <div class="sleep_stage_panel btn-group" role="group"> \
                            <button type="button" class="btn btn-default blue lighten-1 sleep_stage" data-annotation-type="sleep_stage_wake"><span class="shortcut-key">W</span>AKE<span class="shortcut-key hidden">0</span></button> \
                            <button type="button" class="btn btn-default blue lighten-1 sleep_stage" data-annotation-type="sleep_stage_n1">N<span class="shortcut-key">1</span> Sleep</button> \
                        </div> \
                        <div class="submit_response_for_data_panel btn-group" role="group"> \
                            <button type="button" disabled class="btn btn-default submit-response-for-data">SUBMIT ANSWER</button> \
                            <h4 class="phase-indicator right"></h4> \
                        </div> \
    ...
    _setupSleepStagePanel: function() {
        ...
        var inactiveClass = 'blue lighten-1';
        var that = this;
        $(that.element).find('.sleep_stage_panel button.sleep_stage').click(function() {
            $(that.element).find('.submit-response-for-data').prop('disabled', null);
            var button = $(this);
            var type = button.data('annotation-type');
            that._saveSleepStageAnnotation(type);
            that.vars.sleepStageSelected = type;
            button
                .addClass(activeClass)
        ...
        if (that.options.instructionSlidesURL) {
        ...
        if (!that.options.training.enabled) {
            ...
            $(that.element).find('.phase-indicator').html('TRAINING PHASE (WITH FEEDBACK)');
        }
        $(that.element).find('.submit-response-for-data').click(function() {
            $(that.element).find('.sleep_stage_panel button.sleep_stage').prop('disabled', true);
            $(that.element).find('.submit-response-for-data').prop('disabled', true);
            function reloadPage() {
            ...
            var groundTruth = that.options.training.groundTruth || [];
            var groundTruthForWindow = undefined;
            groundTruth.forEach(function(g) {
                if (g.start == that.vars.currentWindowStart) {
                    groundTruthForWindow = g;
                }
            });
            if (!groundTruthForWindow) {
                reloadPage();
                return;
            }
            var makeSleepStageLabelHumanReadable = {
            ...
            if (that.vars.sleepStageSelected == groundTruthForWindow.label) {
                var message = 'Congratulations, your answer is correct! This is indeed "' + makeSleepStageLabelHumanReadable[groundTruthForWindow.label] + '".';
            }
            else {
                var message = 'Unfortunately, your answer is wrong! You selected "' + makeSleepStageLabelHumanReadable[that.vars.sleepStageSelected] + '", but the correct answer is "' + makeSleepStageLabelHumanReadable[groundTruthForWindow.label] + '".';
            }
            message += '\n\n';
            if (groundTruthForWindow.showExplanationVideo && groundTruthForWindow.explanationVideo && groundTruthForWindow.explanationVideo.id) {
                message += 'Click OK to watch an expert discussion about this particular case. After watching the video, you will be automatically taken to the next example.'
            }
            else {
                message += 'Click OK to go to the next step.'
            }
            alert(message);
            if (!groundTruthForWindow.showExplanationVideo || !groundTruthForWindow.explanationVideo || !groundTruthForWindow.explanationVideo.id) {
                reloadPage();
            ...
            var videoHeight = 570;
            var videoBox = bootbox.dialog({
                title: '<b>EXPERT DISCUSSION (PLEASE TURN UP YOUR SOUND)</b>',
                onEscape: false,
                backdrop: false,
                closeButton: false,
                animate: true,
                message: '<div id="expert-discussion-player"></div><div class="interaction-blocker"></div>',
                size: 'large',
            });
            videoBox.appendTo(that.element);
            videoBox.find('.modal-content').css({
                padding: 0,
            })
            videoBox.find('.modal-header').hide();
            videoBox.css({
                backgroundColor: 'rgba(0, 0, 0, 1)',
                zIndex: 999999,
                top: 70,
            ...
            var player = new YTPlayer('#expert-discussion-player', {
                captions: true,
                controls: false,
                fullscreen: true,
                annotations: true,
            ...
            player.load(groundTruthForWindow.explanationVideo.id);
            player.seek(groundTruthForWindow.explanationVideo.start);
            player.setVolume(100);
            player.on('timeupdate', (seconds) => {
                if (seconds >= groundTruthForWindow.explanationVideo.end) {
                    destroyVideoAndLoadNextStep();
                }
            })
            player.on('ended', destroyVideoAndLoadNextStep);
            function destroyVideoAndLoadNextStep() {
                player.destroy();
                videoBox.remove();
